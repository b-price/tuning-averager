...
describe("App Calculation Functions", () => {
    describe("stringAverage", () => {
        it("should return the average note values for each string across tunings", () => {
            expect(stringAverage(testTunings.slice(0, 2))).toEqual([
                51, 46, 42, 37, 32, 27,
            ]);
            expect(stringAverage([testTunings[0], testTunings[0]])).toEqual([
                52, 47, 43, 38, 33, 28,
            ]);
        });
        ...
        it("should return null if tunings do not have the same number of strings", () => {
            expect(
                stringAverage([testTunings[0], testTunings[2]]),
            ).toBeUndefined();
        });

        it("should return null if tunings are for different instruments", () => {
            expect(
                stringAverage([testTunings[1], testTunings[4]]),
            ).toBeUndefined();
        });
    });
    ...
    describe("stringAverageUnweighted", () => {
        it("should return the average note values for each string across tunings, unweighted for occurrence", () => {
            expect(stringAverageUnweighted(testTunings.slice(0, 2))).toEqual([
                51, 46, 42, 37, 32, 27,
            ]);
            expect(
                stringAverageUnweighted([testTunings[0], testTunings[0]]),
            ).toEqual([52, 47, 43, 38, 33, 28]);
            expect(
                stringAverageUnweighted([
                    testTunings[0],
                    testTunings[1],
            ...
            ).toEqual([51, 46, 42, 37, 32, 27]);
        });

        it("should return null if tunings do not have the same number of strings", () => {
            expect(
                stringAverageUnweighted([testTunings[0], testTunings[2]]),
            ).toBeUndefined();
        });
    ...
    describe("convertToNote", () => {
        it("should convert a note value to a note object", () => {
            expect(convertToNote(0)).toStrictEqual({
                note: "C0",
                cents: 0,
                noteValue: 0,
            });
            expect(convertToNote(27)).toStrictEqual({
                note: "D#2",
                cents: 0,
                noteValue: 27,
            });
            expect(convertToNote(1.5)).toStrictEqual({
                note: "C#0",
                cents: 50,
                noteValue: 1.5,
            });
        ...
        it("should return an invalid note if the note value is invalid", () => {
            expect(convertToNote(-1)).toStrictEqual({
                note: "invalid",
                cents: 0,
                noteValue: -1,
            });
            expect(convertToNote(900)).toStrictEqual({
                note: "invalid",
                cents: 0,
                noteValue: 900,
            });
    ...
    describe("getUnitWeight", () => {
        it("calculates unit weight correctly for valid inputs", () => {
            expect(getUnitWeight(57, 25.5, 16)).toBeCloseTo(
                0.0000122775410602,
                12,
            );
        });
    ...
    describe("getPlain", () => {
        ...
        it("returns correct plain string with alt plain", () => {
            expect(getPlain("Daddario_guitar_ClassicSilverplated")).toBe(
                "Daddario_plain_ClearNylon",
            );
        });
    ...
    describe("coeffPower", () => {
        it("returns correct coefficient and power object", () => {
            expect(coeffPower("GHS_guitar_Boomers", true)).toStrictEqual({
                coeff: 2.1627709,
                power: 1.97673144,
            });
            expect(coeffPower("GHS_guitar_Boomers", false)).toStrictEqual({
                coeff: 2.03170652,
                power: 2.04187443,
            });
        });
        it("returns guitar Daddario XL (default) when material does not exist", () => {
            expect(coeffPower("kldflksf", false)).toStrictEqual({
                coeff: 2.03048974,
                power: 1.97385195,
            });
        });
