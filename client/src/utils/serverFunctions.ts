...
export const getUserByUsername = async (username: string) => {
    try {
        const response = await axios.get(
            `${SERVER_URL}/users/?field=username&value=${username}`,
        );
        return { ...response.data, id: response.data._id };
    } catch (error) {
        if (error instanceof AxiosError) {
...
export const getInstruments = async (data?: {
    ...
    try {
        ...
        for (const instrument of data.userData.instruments) {
            const response = await axios.get(
                `${SERVER_URL}/instruments/${instrument}`,
            );
...
export const deleteInstrument = async (instrumentId?: string) => {
    try {
        ...
        const response = await axios.delete(
            `${SERVER_URL}/instruments/${instrumentId}`,
        );
        return response;
    } catch (error) {
        if (error instanceof AxiosError) {
