...
const ExportData: React.FC<ExportDataProps> = ({
    ...
    const [dataSelection, setDataSelection] =
        useState<ExportSelection>("Instruments");
        ...
        switch (dataSelection) {
            ...
            case "StringSets":
                return {
                    ...
                    data: instruments.flatMap((instrument) =>
                        instrument.stringSets
                            .map((strSet) => ({
                                instrument: instrument.name,
                                name: strSet.name,
                                gauges: strSet.gauges
                                    .map((g, idx) =>
                                        !strSet.woundStrings[idx]
                                            ? `${g}${PLAIN_CHAR}`
                                            : g,
                                    )
                                    .join(", "),
                                material: strSet.stringMaterial
                                    ? formatMaterial(strSet.stringMaterial)
                                    : "",
                                favorite: strSet.favorite,
                            }))
                            .filter((strSet) =>
                                onlyFavorites ? strSet.favorite : strSet,
                            ),
                    ),
            ...
            case "Strings": {
                const allGauges = instruments.flatMap((instrument) =>
                    instrument.stringSets
                        .filter((s) => (onlyFavorites ? s.favorite : s))
                        .flatMap((strSet) => strSet.gauges),
                );

                const gaugeCounts = allGauges.reduce(
                    (acc, gauge) => {
            ...
            <div className="flex-col mx-auto sm:px-8 px-4 pb-6 rounded-xl md:max-w-xl">
                ...
                <div>
                    ...
                    <select
                        value={dataSelection}
                        onChange={(e) =>
                            setDataSelection(e.target.value as ExportSelection)
                        }
                    ...
                    <div
                        className={`justify-items-center transition-all duration-300 ease-in-out ${dataSelection !== "Instruments" && dataSelection !== "Tunings" ? "opacity-100" : "opacity-0 h-0 invisible"}`}
                    >
                        <ToggleSwitch
